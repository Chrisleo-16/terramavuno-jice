...
export class IntelHUD {
  ...
  async _summaryContext() {
    // TerraMavuno fork: the GEV voice module (and its Google Places basemap
    // label harvest) was removed. The summary context is now camera metrics +
    // the enabled layer roster; the deterministic fallback line covers the rest.
    ...
    return {
      camera: {
        lat: Number.isFinite(m.latDeg) ? Number(m.latDeg.toFixed(4)) : null,
        lon: Number.isFinite(m.lonDeg) ? Number(m.lonDeg.toFixed(4)) : null,
        altM: Number.isFinite(m.altMslM) ? Math.round(m.altMslM) : null,
      },

...
export function createAnnotationEngine({
  ...
  const engine = {
     ...
     * Lays down a small Kandara "tour" the way the chat/voice agent would.
     * Retargeted from GEV's San Francisco script; targets are place-name
     * strings resolved through the geocoder, so this needs network + a
     * GOOGLE_MAPS_API_KEY. `window.__gevAnnotations.demo()`.
     */
    async demo() {
      return annotate([
        { type: 'area', target: "Kandara, Murang'a, Kenya", label: 'Kandara constituency', color: 'green', footprint: true },
        { type: 'pin', target: "Kabati, Murang'a, Kenya", label: 'Kabati Agrovet (SIMULATED)', color: 'amber' },
        { type: 'highlight', target: 'Sagana, Kirinyaga, Kenya', label: 'NCPB Sagana Depot', color: 'cyan' },
        { type: 'arrow', target: "Kandara, Murang'a, Kenya", toTarget: 'Sagana, Kirinyaga, Kenya', label: 'where to collect' },
    ...
    async tour() {
      clear();
      // Ng'araria ward (K-001's ward) — the calculated centroid from
      // src/data/local_data/kenya/centroids.json.
      flyTo({ lon: 37.02661, lat: -0.93588, height: 6000, heading: 15, pitch: -34, duration: 3 });
      await wait(3200);
      await annotate([{ type: 'area', target: "Kandara, Murang'a, Kenya", label: 'Kandara constituency', color: 'green', footprint: true }], { persist: true });
      await wait(2800);
      await annotate([{ type: 'pin', target: "Kabati, Murang'a, Kenya", label: 'Kabati Agrovet — stock unknown (SIMULATED)', color: 'amber' }], { persist: true });
      await wait(2600);
      flyTo({ lon: 37.10, lat: -0.80, height: 42000, heading: 25, pitch: -50, duration: 3 });
      await wait(3200);
      await annotate([{ type: 'highlight', target: 'Sagana, Kirinyaga, Kenya', label: 'NCPB Sagana Depot — in stock', color: 'cyan' }], { persist: true });
      await wait(2600);
      await annotate([{ type: 'arrow', target: "Kandara, Murang'a, Kenya", toTarget: 'Sagana, Kirinyaga, Kenya', label: 'where to collect', color: 'cyan' }], { persist: true });
      await wait(2600);
      await annotate([{ type: 'route', color: 'amber', label: 'Kandara collection run', points: [
        { target: "Kandara, Murang'a, Kenya" },
        { target: "Kabati, Murang'a, Kenya" },
        { target: 'Sagana, Kirinyaga, Kenya' },
      ] }], { persist: true });
